    ...
    module FormHelper
      ...
      def jqgrid(title, id, action, columns = [], options = {})
        ...
        options = 
          ...
          :rownumbers          => 'false',
          :alt_rows            => 'true',
          :alt_class           => 'ui-priority-secondary',
        ...
        # Show filter toolbar on load -- KD
        if options[:show_filter_toolbar] == 'true'
          filter_toolbar = "mygrid.jqGrid('filterToolbar');"
        end
        ...
        if options[:master_details]
          ...
          onSelectRow: function(ids) { 
            ...
            { 
              jQuery("##{id}_details").jqGrid('setGridParam', { url : "#{options[:details_url]}?q=1&id=" + ids, page : 1 })
              jQuery("##{id}_details").setCaption("#{options[:details_caption]} : "+ids)
        ...
        <script type="text/javascript">
          ...
          var mygrid = jQuery("##{id}").jqGrid({
              ...
              altRows: #{options[:alt_rows]},
              altclass: '#{options[:alt_class]}',
          #{multiselect}
          #{masterdetails}
          #{grid_loaded}
          #{direct_link}
